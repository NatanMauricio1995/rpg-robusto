...
import styles from "./SpellSchoolForm.module.css";
...
export type SpellSchoolFormData = z.infer<typeof spellSchoolSchema>;

interface SpellSchoolFormProps {
  initialData?: Partial<SpellSchool>;
  onSubmit: (data: SpellSchoolFormData) => void;
  onCancel: () => void;
  loading?: boolean;
}

export default function SpellSchoolForm({ 
  initialData, 
  onSubmit, 
  onCancel, 
  loading 
}: SpellSchoolFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<SpellSchoolFormData>({
    resolver: zodResolver(spellSchoolSchema),
    defaultValues: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.container}>
      <div className={styles.formGroup}>
        <label htmlFor="name" className={styles.label}>Nome da Escola</label>
        <input
          id="name"
          type="text"
          {...register("name")}
