...
export const useEncantamentos = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await EncantamentoService.search({ nome: searchTerm });
      setData(result);
    } catch (error) {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      setToast({ message: 'Encantamento salvo!', type: 'success' });
  ...
  const handleConfirmDelete = async () => {
    try {
      setLoading(true);
      await EncantamentoService.delete(selected.id);
      setToast({ message: 'Encantamento excluído!', type: 'success' });
