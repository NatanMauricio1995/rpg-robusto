...
export const useProfiencias = () => {
  ...
  const handleSave = async (formData) => {
    try {
      setLoading(true);
      await ProficienciaService.save(selected?.id, formData);
      setToast({ message: 'Proficiência salva!', type: 'success' });
  ...
  const handleConfirmDelete = async () => {
    try {
      setLoading(true);
      await ProficienciaService.delete(selected.id);
      setToast({ message: 'Proficiência excluída!', type: 'success' });
