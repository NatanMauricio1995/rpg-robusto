...
export const useArmaduras = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await ArmaduraService.search({ nome: searchTerm });
      setData(result);
    } catch (error) {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      setToast({ message: 'Armadura salva!', type: 'success' });
  ...
  const handleConfirmDelete = async () => {
    try {
      setLoading(true);
      await ArmaduraService.delete(selected.id);
      setToast({ message: 'Armadura excluída!', type: 'success' });
