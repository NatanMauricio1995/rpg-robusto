...
export const usePericias = () => {
  ...
  const handleSave = async (formData) => {
    try {
      setLoading(true);
      await PericiaService.save(selected?.id, formData);
      setToast({ message: 'Perícia salva!', type: 'success' });
  ...
  const handleConfirmDelete = async () => {
    try {
      setLoading(true);
      await PericiaService.delete(selected.id);
      setToast({ message: 'Perícia excluída!', type: 'success' });
